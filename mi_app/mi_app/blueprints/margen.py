...
def index():
    if request.method == "POST":
        session['gastos'] = float(request.form.get('gastos', 15330))
        session['pago_movil'] = float(request.form.get('pago_movil', 7190))
        session['envios_al_detal'] = float(request.form.get('envios_al_detal', 170984))
    gastos = session.get('gastos', 15330)
    pago_movil = session.get('pago_movil', 7190)
    envios_al_detal = session.get('envios_al_detal', 170984)
    ...
    fin = fecha + "T23:59:59"
    compras_brs = supabase.table("compras").select("totalprice, amount").eq("fiat", "VES").eq("tradetype", "SELL").gte("createtime", inicio).lte("createtime", fin).execute().data
    brs_comprados = sum(float(c["totalprice"]) for c in compras_brs) if compras_brs else 0
    usdt_vendidos = sum(float(c["amount"]) for c in compras_brs) if compras_brs else 0
    ...
    total_usdt = (float(saldo_anterior["usdt_stock"]) if saldo_anterior and saldo_anterior.get("usdt_stock") is not None else 0) + usdt_comprados
    ...
    total_clp = clp_anterior + clp_invertidos
